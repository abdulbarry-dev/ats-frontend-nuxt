<template>
  ...
  >
    <!-- Hero Section -->
    <div
      class="bg-gradient-to-r from-emerald-600 to-emerald-700 dark:from-emerald-700 dark:to-emerald-800 text-white py-16 sm:py-20 transition-colors duration-300"
    >
      <div class="container mx-auto px-4 max-w-5xl text-center">
        <div
          class="inline-flex items-center justify-center w-16 h-16 bg-white/20 backdrop-blur-sm rounded-2xl mb-6 fade-in-up"
        >
          <Icon name="mdi:help-circle-outline" class="w-8 h-8" />
        </div>
        <h1
          class="text-4xl sm:text-5xl font-bold mb-4 fade-in-up"
          style="animation-delay: 100ms"
        >
          Frequently Asked Questions
        </h1>
        <p
          class="text-emerald-100 dark:text-emerald-50 text-lg max-w-2xl mx-auto fade-in-up transition-colors duration-300"
          style="animation-delay: 150ms"
        >
          Find answers to common questions about FindPoint. Can't find what
    ...
    <div class="container mx-auto px-4 max-w-5xl -mt-8 pb-16">
      <!-- Search Bar -->
      <div
        class="bg-white dark:bg-slate-800 rounded-xl shadow-lg dark:shadow-slate-900/50 border border-gray-200 dark:border-slate-700 p-6 mb-8 fade-in-up transition-colors duration-300"
      ...
      >
        <div class="relative">
          ...
          <input
            ...
            class="w-full pl-12 pr-4 py-4 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white dark:placeholder:text-gray-500 rounded-lg focus:border-emerald-500 dark:focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-400 transition-all outline-none text-lg duration-300"
          />
        </div>
        <p
          v-if="searchQuery && filteredFaqs.length > 0"
          class="mt-3 text-sm text-gray-600 dark:text-gray-400 transition-colors duration-300"
        >
      ...
      <!-- Category Tabs -->
      <div
        class="bg-white dark:bg-slate-800 rounded-xl shadow-sm dark:shadow-slate-900/50 border border-gray-200 dark:border-slate-700 p-4 mb-8 fade-in-up transition-colors duration-300"
      ...
      <!-- Quick Stats -->
      <div
        class="grid sm:grid-cols-3 gap-4 mb-8 fade-in-up"
        style="animation-delay: 300ms"
      >
        <div
          class="bg-gradient-to-br from-emerald-500 to-emerald-600 dark:from-emerald-600 dark:to-emerald-700 rounded-xl p-6 text-white transition-colors duration-300"
        >
          ...
          <p
            class="text-emerald-100 dark:text-emerald-50 transition-colors duration-300"
          >
            Total Questions
          </p>
        </div>
        <div
          class="bg-gradient-to-br from-blue-500 to-blue-600 dark:from-blue-600 dark:to-blue-700 rounded-xl p-6 text-white transition-colors duration-300"
        >
          <div class="flex items-center justify-between mb-2">
            <Icon
          ...
          <p
            class="text-blue-100 dark:text-blue-50 transition-colors duration-300"
          >
            Categories
          </p>
        </div>
        <div
          class="bg-gradient-to-br from-purple-500 to-purple-600 dark:from-purple-600 dark:to-purple-700 rounded-xl p-6 text-white transition-colors duration-300"
        >
          <div class="flex items-center justify-between mb-2">
            <Icon
          ...
          <p
            class="text-purple-100 dark:text-purple-50 transition-colors duration-300"
          >
            Support Available
          </p>
        </div>
      ...
      <div class="space-y-4">
        ...
        <div
          ...
          :key="faq.id"
          class="bg-white dark:bg-slate-800 rounded-xl shadow-sm dark:shadow-slate-900/50 border border-gray-200 dark:border-slate-700 overflow-hidden transition-all duration-300 hover:shadow-md fade-in-up"
          :style="`animation-delay: ${350 + index * 30}ms;`"
        >
          <button
            @click="toggleFaq(faq.id)"
            class="w-full px-6 py-5 flex items-start justify-between text-left hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors group duration-300"
          >
            <div class="flex-1 pr-4">
              <div class="flex items-start gap-3">
                <div
                  :class="[
                    ...
                    openFaqs.includes(faq.id)
                      ...
                      : 'bg-gray-100 dark:bg-slate-700 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/20',
                  ]"
                >
                  <Icon
                ...
                <div class="flex-1">
                  ...
                  <div
                    class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 transition-colors duration-300"
                  >
                    <span
                      class="px-2 py-1 bg-gray-100 dark:bg-slate-700 rounded-md transition-colors duration-300"
                      >{{ getCategoryName(faq.category) }}</span
                    >
                    <span
                      v-if="faq.popular"
                      class="px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-md flex items-center gap-1 transition-colors duration-300"
                    >
            ...
            <div
              :class="[
                'w-8 h-8 rounded-lg flex items-center justify-center transition-all flex-shrink-0 duration-300',
                openFaqs.includes(faq.id)
                  ? 'bg-emerald-600 dark:bg-emerald-500 rotate-180'
                  : 'bg-gray-100 dark:bg-slate-700 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-900/30',
              ]"
            >
              <Icon
          ...
          <Transition name="expand">
            <div v-if="openFaqs.includes(faq.id)" class="px-6 pb-6">
              <div class="pl-[52px] pr-4">
                <div
                  class="p-4 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-slate-700 dark:to-slate-600 rounded-lg border border-gray-200 dark:border-slate-600 transition-colors duration-300"
                >
                  <p
                    class="text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-line transition-colors duration-300"
                  >
                    {{ faq.answer }}
                  </p>
                  <div
                    v-if="faq.links && faq.links.length > 0"
                    class="mt-4 pt-4 border-t border-gray-300 dark:border-slate-500 transition-colors duration-300"
                  >
                    <p
                      class="text-sm font-semibold text-gray-900 dark:text-white mb-2 transition-colors duration-300"
                    >
                      Related Links:
                    </p>
                    <div class="flex flex-wrap gap-2">
                      <NuxtLink
                        ...
                        :key="idx"
                        :to="link.url"
                        class="inline-flex items-center gap-1 text-sm text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 font-medium transition-colors duration-300"
                      >
                ...
                <div class="mt-4 flex items-center justify-between">
                  ...
                  <div class="flex gap-2">
                    <button
                      ...
                      class="px-4 py-2 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-lg text-sm font-medium hover:bg-emerald-200 dark:hover:bg-emerald-900/50 transition-colors flex items-center gap-1 duration-300"
                    >
                    ...
                    <button
                      ...
                      class="px-4 py-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors flex items-center gap-1 duration-300"
                    >
      ...
      <div
        class="mt-12 bg-gradient-to-br from-emerald-600 to-emerald-700 dark:from-emerald-700 dark:to-emerald-800 rounded-2xl p-8 sm:p-12 text-center text-white fade-in-up transition-colors duration-300"
      >
        <div
        ...
        <p
          class="text-emerald-100 dark:text-emerald-50 mb-6 max-w-2xl mx-auto transition-colors duration-300"
        >
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          <NuxtLink
            to="/contact"
            class="px-8 py-4 bg-white text-emerald-600 dark:text-emerald-700 rounded-lg font-semibold hover:bg-gray-100 dark:hover:bg-gray-100 transition-colors inline-flex items-center justify-center gap-2 duration-300"
          >
          ...
          <NuxtLink
            to="/help"
            class="px-8 py-4 bg-emerald-800/50 dark:bg-emerald-900/50 backdrop-blur-sm text-white rounded-lg font-semibold hover:bg-emerald-800/70 dark:hover:bg-emerald-900/70 transition-colors inline-flex items-center justify-center gap-2 duration-300"
          >
...
useSeoMeta({
  ...
  description:
    "Find answers to common questions about FindPoint job platform, applications, and more.",
});
...
const faqs = ref<FAQ[]>([
  {
    ...
    answer:
      "FindPoint is a comprehensive job search platform that connects talented professionals with amazing career opportunities. We help job seekers discover their perfect role and assist companies in finding the right talent for their teams.\n\nOur platform offers advanced search filters, personalized job recommendations, application tracking, and direct communication with recruiters.",
    popular: true,
    links: [
  ...
  },
  {
    ...
    popular: true,
    links: [{ text: "Sign Up Now", url: "/auth/signup" }],
  },
  {
    id: 3,
    category: "jobs",
  ...
  },
  {
    ...
    popular: false,
    links: [{ text: "Edit Profile", url: "/candidates/profile" }],
  },
  {
    id: 7,
    category: "jobs",
  ...
  },
  {
    ...
    popular: false,
    links: [{ text: "Account Settings", url: "/settings/account" }],
  },
  {
    id: 12,
    category: "general",
