<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-2">My Applications</h1>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-all duration-300">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-gradient-to-br from-emerald-100 to-emerald-200 rounded-lg flex items-center justify-center">
              <Icon name="mdi:file-document-multiple" class="w-5 h-5 text-emerald-600" />
            </div>
            <div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-all duration-300">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="text-2xl font-bold text-gray-900">{{ statusCount('under-review') }}</div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-all duration-300">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-gradient-to-br from-amber-100 to-amber-200 rounded-lg flex items-center justify-center">
              <Icon name="mdi:calendar-check" class="w-5 h-5 text-amber-600" />
            </div>
            <div>
              <div class="text-2xl font-bold text-gray-900">{{ statusCount('interview') }}</div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-all duration-300">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-gradient-to-br from-purple-100 to-purple-200 rounded-lg flex items-center justify-center">
              <Icon name="mdi:star-circle" class="w-5 h-5 text-purple-600" />
            </div>
            <div>
              <div class="text-2xl font-bold text-gray-900">{{ statusCount('offer') }}</div>
        ...
        <div class="flex flex-col lg:flex-row gap-4">
          ...
          <div class="relative flex-1">
            ...
            <input 
              ...
              class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all"
            />
          </div>
          ...
          <select 
            v-model="sortBy" 
            class="px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all cursor-pointer"
          >
      ...
      <!-- Applications List -->
      <div v-if="filteredApplications.length === 0" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
        <div class="w-24 h-24 bg-gradient-to-br from-gray-100 to-gray-200 rounded-full flex items-center justify-center mx-auto mb-6">
          <Icon name="mdi:briefcase-search-outline" class="w-12 h-12 text-gray-400" />
        </div>
        <h3 class="text-xl font-bold text-gray-900 mb-2">No applications found</h3>
        <p class="text-gray-600 mb-6 max-w-md mx-auto">
          {{ search ? 'Try adjusting your search or filters' : 'Start applying to jobs to see them here' }}
        </p>
        <NuxtLink 
          to="/jobs" 
          ...
          <div class="p-6">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              ...
              <div class="flex items-start gap-4 flex-1">
                <!-- Company Logo -->
                <div class="w-16 h-16 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center flex-shrink-0 border border-gray-200 overflow-hidden group-hover:scale-105 transition-transform duration-300">
                  <img v-if="app.logo" :src="app.logo" :alt="app.company + ' logo'" class="object-contain w-full h-full" />
                  <div v-else class="text-emerald-700 font-bold text-lg">{{ initials(app.company) }}</div>
                </div>
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex flex-wrap items-center gap-3 text-sm">
                    ...
                    <span v-if="app.salary" class="flex items-center gap-1.5 text-emerald-600 font-semibold">
                  ...
                  <div v-if="app.status === 'interview' || app.status === 'offer'" class="mt-4 pt-4 border-t border-gray-100">
                    <div class="flex items-center gap-2">
                      ...
                      <div class="flex items-center gap-1">
                        ...
                        <div class="text-xs font-medium text-gray-700">Review</div>
                      </div>
                      <div :class="['h-0.5 w-8', app.status === 'offer' ? 'bg-emerald-600' : 'bg-gray-200']"></div>
                      <div class="flex items-center gap-1">
                        <div :class="['w-6 h-6 rounded-full flex items-center justify-center', app.status === 'offer' ? 'bg-emerald-600' : 'bg-amber-600']">
                          <Icon :name="app.status === 'offer' ? 'mdi:check' : 'mdi:calendar'" class="w-4 h-4 text-white" />
                        </div>
                        <div class="text-xs font-medium text-gray-700">Interview</div>
                      </div>
                      <div :class="['h-0.5 w-8', app.status === 'offer' ? 'bg-emerald-600' : 'bg-gray-200']"></div>
                      <div class="flex items-center gap-1">
                        <div :class="['w-6 h-6 rounded-full flex items-center justify-center', app.status === 'offer' ? 'bg-emerald-600' : 'bg-gray-200']">
                          <Icon name="mdi:trophy" class="w-4 h-4 text-white" />
                        </div>
                        <div class="text-xs font-medium text-gray-700">Offer</div>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      >
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all">
          ...
          <p class="text-gray-600 mb-6">
            Are you sure you want to withdraw your application? You won't be able to recover it.
          </p>
...
const statusClass = (status: string) => {
  switch (status) {
    ...
    case 'interview': return 'bg-amber-100 text-amber-700 border border-amber-200'
    case 'rejected': return 'bg-red-100 text-red-700 border border-red-200'
    case 'offer': return 'bg-emerald-100 text-emerald-700 border border-emerald-200'
...
const confirmWithdraw = () => {
  if (applicationToWithdraw.value) {
    const idx = applications.value.findIndex(a => a.id === applicationToWithdraw.value)
    if (idx > -1) {
...
useHead({ 
  ...
  meta: [
    { name: 'description', content: 'Track and manage all your job applications' }
  ]
})
</script>
